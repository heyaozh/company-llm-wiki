...
# Lint Wiki (agent-side validation)

**FIRST, read the Wiki Contract.** Read the **SCHEMA** and **AGENT** pages in the space via the
**Confluence MCP**; validate against them. Working language: **English**.

There is **no CI** — this skill IS the gate. Run it (a) pre-publish inside `ingest-source`
step 6 on the page about to be published, and (b) on demand across the whole space as an audit.
REPORT only — never auto-delete or auto-fix silently.
